...
if(defined($owner) and defined($title)){
  ...
  if(@articlesTex){
    ...
    print STDERR " no se encontAre su texto, revise sus datos\n";
  }
}else{
...
sub interpretar{
  ...
  if($linea =~ /(^\`\`\`)/){  
    ...
    if($lineas[$i+$j] =~ /(^\`\`\`)/){
      $grupoHTML ="<pre><code>$lineaHTML</code></pre>";
...
sub evaluarAter{
  ...
  my $final = "";
  my @linea;
  if ($text =~ /(.+)([*])(.+)([*])(.*)/){ #el corchete lo vuelve caracter y no es cuantificador
    $inicio .= $1;
    print STDERR "prim$1";
